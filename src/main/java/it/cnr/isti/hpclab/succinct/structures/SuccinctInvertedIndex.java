...
public class SuccinctInvertedIndex implements PostingIndex<BitIndexPointer>
{
	...
	public IterablePosting getPostings(BitIndexPointer pointer) throws IOException 
	{
		...
		LongBigList docidsList = ByteBufferLongBigList.map( new FileInputStream( index.getPath() + File.separator + index.getPrefix() + ".docids" ).getChannel(), byteOrder, MapMode.READ_ONLY );
		LongBigList freqsList  = ByteBufferLongBigList.map( new FileInputStream( index.getPath() + File.separator + index.getPrefix() + ".freqs" ).getChannel(), byteOrder, MapMode.READ_ONLY );
